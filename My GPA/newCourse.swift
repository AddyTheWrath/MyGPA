...
class newCourse: UITableViewController {
    ...
    @IBAction func saveButtonPressed(_ sender: Any) {
        ...
        if let name = courseNameText.text, let desiredGradeAsString = desiredGradeText.text {
            ...
            if  (desiredGrade == nil || desiredGrade! < 0 || desiredGrade! > 100) {
                ...
                delegate?.controller(controller: self, didSaveCourseWithName: name, andCredit: credit, andDesiredGrade: desiredGrade!, andCountToGPA: countToGPA)
                // Notify Delegate
